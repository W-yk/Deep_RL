...
class DDPG_agent:
    ...
    def __init__(self, env):
        ...
        self.s = tf.placeholder(tf.float32, [None, self.state_size[0],self.state_size[1],self.state_size[2]], "state")
    ...
    def create_Actor_model(self, scope, lr=0.01):
        with tf.variable_scope("Actor"):
            ...
            l2 = tf.layers.conv2d(self.s, 24,[4,4],activation= tf.nn.relu, name="conv1")
            l3 = tf.layers.conv2d(l2, 24,[4,4], activation=tf.nn.relu, name="conv2")
            l3_= tf.layers.flatten(l3)
            a0 = tf.layers.dense(l3_, 1, tf.nn.tanh, name="steer")
            a12 = tf.layers.dense(l3_, self.action_size-1, tf.nn.relu, name="gas_break")
            actions = tf.concat([a0,a12], axis=-1, name="actions" )
            #actions = tf.layers.dense(l3, self.action_size,tf.nn.relu,  name="actions")
    ...
    def create_Critic_model(self, scope, lr=0.01):
        with tf.variable_scope("Critic"):
            ...
            l2 = tf.layers.conv2d(self.s,24,[4,4], activation=tf.nn.relu, name="State_Dense1")
            l2_ = tf.layers.flatten(l2)
            l3 = tf.layers.dense(self.a, 24, activation=tf.nn.relu, name="Action_Dense1")
            l4 = tf.concat([l2_, l3], axis=-1)
    ...
    def Actor_learn(self, states, grads):
        ...
        self.sess.run([self.actions, self.Atrain],
                      feed_dict={self.s: states, self.action_grads: np.squeeze(grads), self.training: True})
    ...
    def replay(self, batch_size):
        ...
        States = np.empty((batch_size, self.state_size[0],self.state_size[1],self.state_size[2]))
...
if __name__ == "__main__":
    ...
    def train(agent, Episodes=500, batch_size=32):
        ...
        try:
            #saver.restore(agent.sess, "save/weights")
            print("load weights")
        except:
            print("weights not found")
        for e in range(Episodes):
            state = np.reshape(agent.env.reset(), [1, agent.state_size[0],agent.state_size[1],agent.state_size[2]])
            ...
            for time in range(1000):
                ...
                next_state = np.reshape(next_state, [1, agent.state_size[0],agent.state_size[1],agent.state_size[2]])
        ...
        saver.save(agent.sess, "save/weights")
